...
func ExportMacroDictionaryExample() {
	...
	fmt.Println("1. Inicializando dados internos (carregando mapas de caracteres e dicionários de macros)...")
	err := reader.InitializeInternals()
	if err != nil {
		log.Printf("Erro ao inicializar dados internos: %v", err)
		return
	}
	fmt.Println("   ✓ Dados internos carregados com sucesso")
...
func BinaryReconstructionExample() {
	fmt.Println("=== Exemplo de Reconstrução Binária de Macro Dictionary ===")
	...
	fmt.Println("\n3. Testando conversão completa (JSON → Binary → JSON)...")
	writer.TestMacroStringReconstruction()

	// Step 4: Write binary files
	fmt.Println("\n4. Criando arquivos binários reconstruídos...")
	writer.WriteMacroDictionaryToBinaryFiles()

	fmt.Println("\n✓ Exemplo de reconstrução binária concluído!")
	fmt.Println("Arquivos criados em:")
	fmt.Println("  - edits/macrodic/*.json (formatos JSON)")
	fmt.Println("  - edits/macrodic/binary/*.dcp (formatos binários reconstruídos)")
}
