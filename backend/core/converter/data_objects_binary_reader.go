...
func PopulateDataObjectLocalizationsWithIlist(path string, objects components.IList[components.ILocalizedTextObject], creator func([]byte, []byte, int, string) components.ILocalizedTextObject) {
	...
	for locKey := range common.SupportedLanguages {
		...
		if localizationData != nil {
			maxLen := min(localizationData.GetLength(), objects.GetLength())
			items := objects.GetItems()
			locItems := localizationData.GetItems()
			for i := range maxLen {
...
func ParseDataListWithIlist(data []byte, languageCode string, creator func([]byte, []byte, int, string) components.ILocalizedTextObject) components.IList[components.ILocalizedTextObject] {
