...
func (d *DcpFile) Extract() error {
	...
	if err := d.ensureDcpPartsLength(expectedDcpPartsLength); err != nil {
		d.LogError(err, "failed to unpack DCP file: %s", d.Source().Get().Name)

		return fmt.Errorf("failed to unpack DCP file: %s", d.Source().Get().Name)
	}

	if d.PartsList.GetLength() != expectedDcpPartsLength {
		d.LogError(nil, "invalid number of split files: Expected: %d, Get: %d", expectedDcpPartsLength, d.PartsList.GetLength())
	...
	extractParts := func(_ int, part parts.DcpFileParts) {
		if err := part.Validate(); err != nil {
			d.LogError(err, "failed to validate file part: %s", part.Source().Get().Name)

			errChan <- fmt.Errorf("failed to validate file part: %s", part.Source().Get().Name)
		}

		if err := part.Extract(); err != nil {
			d.LogError(err, "failed to extract file part: %s", part.Source().Get().Name)
	...
	if err := d.dcpFileVerify.VerifyExtract(d.PartsList, d.options); err != nil {
		d.LogError(err, "failed to verify DCP file: %s", d.Source().Get().Name)

		return fmt.Errorf("failed to verify DCP file: %s", d.Source().Get().Name)
	}

	d.LogInfo("System macrodic file extracted: %s", d.Source().Get().Name)
...
func (d *DcpFile) Compress() error {
	...
	d.LogInfo("Compressing DCP file parts...")

	expectedDcpPartsLength := d.options.PartsLength

	if err := d.ensureDcpPartsLength(expectedDcpPartsLength); err != nil {
		d.LogError(err, "failed to packing DCP file: %s", d.Source().Get().Name)
	...
	if err := components.ListFilesByRegex(dcpSplitedTextFiles, dcpTranslatedPartsTextPath, lib.DCP_TXT_PARTS_PATTERN); err != nil {
		d.LogError(err, "error listing xplited text files: %s", dcpTranslatedPartsTextPath)
	...
	if dcpSplitedTextFiles.GetLength() != expectedDcpPartsLength {
		d.LogError(nil, "invalid number of split text files: Expected: %d, Get: %d, Path: %s", expectedDcpPartsLength, dcpSplitedTextFiles.GetLength(), dcpTranslatedPartsTextPath)
	...
	compressor := func(_ int, part parts.DcpFileParts) {
		if err := part.Compress(); err != nil {
			d.LogError(err, "failed to compress file part: %s", part.Source().Get().Name)
		}
	}
	...
	if err := joinner.DcpFileJoiner(d.Source(), d.Destination(), d.PartsList, outputFile); err != nil {
		d.LogError(err, "error joining macrodic file: %s", outputFile)
	...
	d.LogInfo("Macrodic file compressed: %s", d.Source().Get().Name)
