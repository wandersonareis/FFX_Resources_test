...
var _ = Describe("LocalizedFieldStringObject", func() {
	...
	Describe("NewLocalizedFieldStringObjectWithContent", func() {
		It("should create object with initial content", func() {
			fieldString := event.NewFieldString(charset, 0x0010, 0x00, 0x00, 0x0010, 0x00, 0x00, testBytes, common.GameVersionFFX)
	...
	Describe("SetLocalizedContent", func() {
		Context("when setting new content", func() {
			...
			It("should overwrite existing non-empty content", func() {
				// Set initial content
				fieldString1 := event.NewFieldString(charset, 0x0010, 0x00, 0x00, 0x0010, 0x00, 0x00, testBytes, common.GameVersionFFX)
				obj.SetLocalizedContent("us", fieldString1)

				// Set new content
				fieldString2 := event.NewFieldString(charset, 0x0011, 0x00, 0x00, 0x0011, 0x00, 0x00, testBytes, common.GameVersionFFX)
	...
	Describe("ReadAndSetLocalizedContent", func() {
		It("should read and set content from byte data", func() {
			obj.ReadAndSetLocalizedContent("us", testBytes, 0x0008, 0x00, 0x00, 0x000C, 0x00, 0x00, common.GameVersionFFX)
		...
		It("should handle nil bytes gracefully", func() {
			obj.ReadAndSetLocalizedContent("us", nil, 0x0010, 0x00, 0x00, 0x0014, 0x00, 0x00, common.GameVersionFFX)
	...
	Describe("GetLocalizedString", func() {
		It("should return string for existing localization", func() {
			fieldString := event.NewFieldString(charset, 0x0008, 0x00, 0x00, 0x0008, 0x00, 0x00, testBytes, common.GameVersionFFX)
			obj.SetLocalizedContent("us", fieldString)
	...
	Describe("CopyInto", func() {
		It("should copy all content into another object", func() {
			// Setup source object
			fieldString1 := event.NewFieldString(charset, 0x0010, 0x00, 0x00, 0x0010, 0x00, 0x00, testBytes, common.GameVersionFFX)
			fieldString2 := event.NewFieldString(charset, 0x0014, 0x00, 0x00, 0x0014, 0x00, 0x00, testBytes, common.GameVersionFFX)
	...
	Describe("WriteAllContent", func() {
		It("should format all content with localization names", func() {
			...
			fieldString1 := event.NewFieldString(charset, 0x0008, 0x00, 0x00, 0x0008, 0x00, 0x00, testBytes, common.GameVersionFFX)
			fieldString2 := event.NewFieldString(charset, 0x000C, 0x00, 0x00, 0x000C, 0x00, 0x00, testBytes, common.GameVersionFFX)
