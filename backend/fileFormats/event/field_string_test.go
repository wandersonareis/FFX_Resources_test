...
var _ = Describe("FieldString", func() {
	...
	Describe("FromFieldStringData", func() {
		Context("when parsing field string data", func() {
			It("should correctly parse multiple field strings", func() {
				strings, err := event.FromFieldStringData(testBytes, charset, common.GameVersionFFX)
			...
			It("should handle empty byte arrays", func() {
				strings, err := event.FromFieldStringData([]byte{}, charset, common.GameVersionFFX)
	...
	Describe("RebuildFieldStrings", func() {
		Context("when rebuilding field strings", func() {
			var fieldStrings []*event.FieldString
			...
			BeforeEach(func() {
				...
				fieldStrings = []*event.FieldString{fs1, fs2}
			})

			It("should rebuild strings into byte format", func() {
				result := event.RebuildFieldStrings(fieldStrings, charset, common.GameVersionFFX)
			...
			It("should handle empty field strings list", func() {
				result := event.RebuildFieldStrings([]*event.FieldString{}, charset, common.GameVersionFFX)
	...
	Describe("LocalizedFieldStringObject", func() {
		var (
			localizedObj *event.LocalizedFieldStringObject
		...
		BeforeEach(func() {
			setupFieldStringCharMaps()
			localizedObj = event.NewLocalizedFieldStringObject()
		...
		Describe("NewLocalizedFieldStringObject", func() {
			...
			It("should create an object with initial content", func() {
				obj := event.NewLocalizedFieldStringObjectWithContent("us", testFS1)
				Expect(obj).ToNot(BeNil())
		...
		Describe("CopyInto", func() {
			It("should copy all content to another object", func() {
				...
				otherObj := event.NewLocalizedFieldStringObject()
