...
export class GameTextParserService {
  /**
  ...
  private classifyColorTag(inner: string): { hex: string | null; isReset: boolean } | null {
  ...
  private classifyFontTag(inner: string): 'italic' | 'normal' | null {
    const upper = inner.trim().toUpperCase();
  ...
  private isChipTag(inner: string): boolean {
    const upper = inner.trim().toUpperCase();
  ...
  private wrapStyledText(text: string, color: string | null, italic: boolean): string {
